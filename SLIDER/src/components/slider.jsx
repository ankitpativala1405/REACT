...
export default function EventsWithPagination() {
    ...
    const [textContent, setTextContent] = useState(trimer(productList[0].description, 100));
        ...
        <div className='flex items-center mx-auto gap-[40px] p-[20px] text-center max-w-full flex-col md:flex-row'>
            ...
            <div className='text-white md:w-[1000px] w-full max-w-full h-[425px] border-[#ddd] rounded-2xl p-10 shadow-2xl overflow-hidden bg-[#121212] flex flex-col md:flex-row items-center gap-6'>
                ...
                <div className='flex flex-col flex-grow justify-between h-full max-w-[650px]'>
                    <p
                        className={`m-0 text-[#bbb] text-[1rem] text-left transition-opacity duration-700 ease-in-out ${showText ? 'opacity-100' : 'opacity-0'
                            }`}
                        aria-live="polite"
                    >
                        {textContent}
                    ...
                    <div className='pt-4'>
                        <a href="#" className="text-[#eee] hover:text-blue-400 transition-colors" tabIndex={-1}>
                            <h3 className='text-[1.5rem] font-semibold'>{currentProduct.title}</h3>
                        </a>
                    </div>
                </div>
            </div>
